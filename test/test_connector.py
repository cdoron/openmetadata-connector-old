...
response = requests.post("http://localhost:8081/createAsset", json=asset_data, headers=headers)
...
response = requests.post("http://localhost:8081/getAssetInfo", json=data, headers=headers)
assert response.status_code == 200
...
response = requests.patch("http://localhost:8081/updateAsset", json=data, headers=headers)
assert response.status_code == 200
...
response = requests.delete("http://localhost:8081/deleteAsset", json=data, headers=headers)
assert response.status_code == 200
